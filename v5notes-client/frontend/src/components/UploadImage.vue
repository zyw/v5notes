<template>
  ...
  import { getServerUrl, generateUUID } from "@/utils/commons";
  ...
  interface IUploadImageProps {
    ...
    showLabel?: boolean; // 鼠标浮上是否显示label
  }
  
  // 接收父组件参数
  const props = withDefaults(defineProps<IUploadImageProps>(), {
    imageUrl: "",
    action: '/basics/base/file',
    drag: true,
    disabled: false,
    fileSize: 3,
  ...
  const handleHttpUpload = async (options: UploadRequestOptions) => {
    ...
    try {
      ...
      const serverUrl = getServerUrl() + props.action
      const res: any = await service.post(serverUrl, formData);
