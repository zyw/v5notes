...
import org.dromara.common.oss.constant.FileConstant;
...
public class FileOssClient {
     ...
     * 构造方法
     *
     * @param fileProperties Oss配置属性
     */
    public FileOssClient(FileProperties fileProperties) {
        ...
        try {
            ...
            boolean isStyle = !StringUtils.containsAny(properties.getEndpoint(), FileConstant.CLOUD_SERVICE);
            ...
            throw new FileException("配置错误! 请检查系统配置:[" + e.getMessage() + "]");
        }
    }
     ...
     *
     * @throws FileException 当创建存储桶时发生异常时抛出
     */
        ...
        try {
            ...
            if (ex.getCause() instanceof NoSuchBucketException) {
                try {
                    ...
                    // 存储桶创建或策略设置失败
                    throw new FileException("创建Bucket失败, 请核对配置信息:[" + e.getMessage() + "]");
                }
            } else {
                throw new FileException("判断Bucket是否存在失败，请核对配置信息:[" + ex.getMessage() + "]");
            }
        }
    }
     ...
     * @throws FileException 如果上传失败，抛出自定义异常
     */
        ...
        try {
            ...
            FileUpload fileUpload = transferManager.uploadFile(
                x -> x.putObjectRequest(
                        y -> y.bucket(properties.getBucket())
                            .key(key)
            ...
            return UploadResult.builder().url(getUrl() + StringUtils.SLASH + key).path(key).eTag(eTag).build();
        } catch (Exception e) {
            // 捕获异常并抛出自定义异常
            throw new FileException("上传文件失败，请检查配置信息:[" + e.getMessage() + "]");
        } finally {
            // 无论上传是否成功，最终都会删除临时文件
     ...
     * @throws FileException 如果下载失败，抛出自定义异常
     */
    ...
    public long download(String key, OutputStream out) {
        try {
            ...
            DownloadRequest<ResponseInputStream<GetObjectResponse>> downloadRequest = DownloadRequest.builder()
                // 文件对象
                .getObjectRequest(y -> y.bucket(properties.getBucket())
                    .key(key)
                    .build())
                .addTransferListener(LoggingTransferListener.create())
                // 使用订阅转换器
            ...
            throw new FileException("文件下载失败，错误信息:[" + e.getMessage() + "]");
        }
    }
    ...
    public void delete(String path) {
        try {
            client.deleteObject(
                x -> x.bucket(properties.getBucket())
                    .key(removeBaseUrl(path))
                    .build());
        } catch (Exception e) {
            throw new FileException("删除文件失败，请检查配置信息:[" + e.getMessage() + "]");
        }
    }
    ...
    public String getDomain() {
        ...
        // 如果是云服务商，直接返回域名或终端点
        if (StringUtils.containsAny(endpoint, FileConstant.CLOUD_SERVICE)) {
    ...
    public String getUrl() {
        ...
        // 云服务商直接返回
        if (StringUtils.containsAny(endpoint, FileConstant.CLOUD_SERVICE)) {
            return header + (StringUtils.isNotEmpty(domain) ? domain : properties.getBucket() + "." + endpoint);
        }
        // MinIO 单独处理
        if (StringUtils.isNotEmpty(domain)) {
            // 如果 domain 以 "https://" 或 "http://" 开头
            return (domain.startsWith(Constants.HTTPS) || domain.startsWith(Constants.HTTP)) ?
                domain + StringUtils.SLASH + properties.getBucket() : header + domain + StringUtils.SLASH + properties.getBucket();
        }
        return header + endpoint + StringUtils.SLASH + properties.getBucket();
    }
...
//    }
    ...
    public String getIsHttps() {
        return FileConstant.IS_HTTPS.equals(properties.getIsHttps()) ? Constants.HTTPS : Constants.HTTP;
    }

    /**
     * 检查配置是否相同
     */
    public boolean checkPropertiesSame(FileProperties properties) {
