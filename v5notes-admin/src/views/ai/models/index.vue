        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            ...
            <el-form-item label="函数调用方式：0：不支持，1：Function Call，2：Tool Call" prop="functionCalling">
              <el-input v-model="queryParams.functionCalling" placeholder="请输入函数调用方式：0：不支持，1：Function Call，2：Tool Call" clearable @keyup.enter="handleQuery" />
            </el-form-item>
            <el-form-item label="流式函数调用：0：不支持，1：支持" prop="streamFunctionCalling">
              <el-input v-model="queryParams.streamFunctionCalling" placeholder="请输入流式函数调用：0：不支持，1：支持" clearable @keyup.enter="handleQuery" />
            </el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['system:models:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['system:models:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
...
import { listModels, getModels, delModels, addModels, updateModels } from '@/api/system/models';
import { ModelsVO, ModelsQuery, ModelsForm } from '@/api/system/models/types';
...
const data = reactive<PageData<ModelsForm, ModelsQuery>>({
  ...
  rules: {
    ...
    supplierId: [
      { required: true, message: "供应商ID，所属供应商不能为空", trigger: "blur" }
    ],
    modelId: [
      { required: true, message: "模型ID不能为空", trigger: "blur" }
    ],
    name: [
      { required: true, message: "模型展示名称不能为空", trigger: "blur" }
    ],
    type: [
      { required: true, message: "模型类型：1:LLM,2:Text Embedding,3:Rerank,4:TTS,5:Speech2text,6:Moderation不能为空", trigger: "change" }
    ],
    contextLen: [
      { required: true, message: "模型上下文长度不能为空", trigger: "blur" }
    ],
    maxToken: [
      { required: true, message: "最大token不能为空", trigger: "blur" }
    ],
    functionCalling: [
      { required: true, message: "函数调用方式：0：不支持，1：Function Call，2：Tool Call不能为空", trigger: "blur" }
    ],
    streamFunctionCalling: [
      { required: true, message: "流式函数调用：0：不支持，1：支持不能为空", trigger: "blur" }
    ],
    vision: [
      { required: true, message: "是否视觉支持：0：不支持，1：支持不能为空", trigger: "blur" }
    ],
    status: [
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('system/models/export', {
    ...queryParams.value
